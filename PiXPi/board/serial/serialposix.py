...
import board.serial
from board.serial.serialutil import SerialBase, SerialException, to_bytes, \
...
class Serial(SerialBase, PlatformSpecific):
    ...
    def _reconfigure_port(self, force_update=False):
        ...
        # setup stop bits
        if self._stopbits == board.serial.STOPBITS_ONE:
            cflag &= ~(termios.CSTOPB)
        elif self._stopbits == board.serial.STOPBITS_ONE_POINT_FIVE:
            cflag |= (termios.CSTOPB)  # XXX same as TWO.. there is no POSIX support for 1.5
        elif self._stopbits == board.serial.STOPBITS_TWO:
            cflag |= (termios.CSTOPB)
        else:
            raise ValueError('Invalid stop bit specification: {!r}'.format(self._stopbits))
        # setup parity
        iflag &= ~(termios.INPCK | termios.ISTRIP)
        if self._parity == board.serial.PARITY_NONE:
            cflag &= ~(termios.PARENB | termios.PARODD | CMSPAR)
        elif self._parity == board.serial.PARITY_EVEN:
